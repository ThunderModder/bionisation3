...
public class EffectEventHandler {
    ...
    public void onBookUnlock(PlayerEvent.PlayerLoggedInEvent event){
        ...
        if(!player.world.isRemote){
            for(Map.Entry<ResourceLocation, IRecipe> entry : ForgeRegistries.RECIPES.getEntries()){
                if(entry.getKey().toString().startsWith(Information.MOD_ID)){
    ...
    public void onEntityAttacked(LivingHurtEvent event) {
        ...
        if(!world.isRemote){
            if(target instanceof EntityPlayer){
                ...
                IBioPlayer cap = player.getCapability(BioPlayerProvider.BIO_PLAYER_CAPABILITY, null);
                //EFFECT BLEEDING
                if(source.getTrueSource() instanceof EntityLivingBase && getRandom(CHANCE_BLEEDING))
                ...
                if(source.getTrueSource() instanceof EntityBlaze && getRandom(CHANCE_GLOWINGBACTERIABL))
                ...
                //BONE BACTERIA
                if((source.getTrueSource() instanceof EntitySkeleton || source.getTrueSource() instanceof EntityStray) && getRandom(CHANCE_BONEBACTERIA))
                ...
                if(source.getTrueSource() instanceof EntityCreeper && getRandom(CHANCE_SMALLGREENBACTERIA)) {
                    fromCreeper = true;
                ...
                //BLOOD VIRUS
                if(source.getTrueSource() instanceof EntityWitch && isEffectActive(ID_BLOOD_VIRUS, (EntityLivingBase)source.getTrueSource()))
                   cap.addEffect(new BloodVirus(STANDART_VIRUS_DURATION, 1), player);
                //SKULL VIRUS
                if((source.getTrueSource() instanceof EntitySkeleton || source.getTrueSource() instanceof EntityStray) && isEffectActive(ID_SKULL_VIRUS, (EntityLivingBase)source.getTrueSource()))
                    cap.addEffect(new SkullVirus(DURATION_SKULL_VIRUS, 1), player);
                //POLAR VIRUS
                if(source.getTrueSource() instanceof EntityPolarBear && getRandom(CHANCE_POLAR_VIRUS))
                ...
                //EFFECT BLEEDING
                if(source.getTrueSource() instanceof EntityLivingBase && getRandom(CHANCE_BLEEDING * 3))
            ...
            //ALL OTHER
            //WITHER VIRUS
            if((source.getTrueSource() instanceof EntityWitherSkull || source.getTrueSource() instanceof EntityWitherSkeleton) && getRandom(CHANCE_WITHER_VIRUS))
    ...
    public void onEntityAttack(LivingAttackEvent event) {
        ...
        if(!world.isRemote){
            if(dsource.getTrueSource() instanceof EntityPlayer){
                EntityPlayer player = (EntityPlayer)dsource.getTrueSource();
                IBioPlayer cap = player.getCapability(BioPlayerProvider.BIO_PLAYER_CAPABILITY, null);
                //BLACK BACTERIA
