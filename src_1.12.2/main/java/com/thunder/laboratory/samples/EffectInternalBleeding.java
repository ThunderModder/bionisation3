...
public class EffectInternalBleeding extends AbstractEffect {

    public EffectInternalBleeding() {
        this(Constants.DURATION_BLEEDING, 0);
    }

    public EffectInternalBleeding(int duration, int power) {
        super(Constants.ID_INTERNAL_BLEEDING, duration, power, false, "Internal Bleeding", SampleType.EFFECT);
    ...
    public void performEntity(Event event, EntityLivingBase entity, EventType type, IBioMob cap) {
        if(type == EventType.TICK) {
            if(Utilities.isTickerEqual(cap.getTicker(), 300))
                entity.attackEntityFrom(DamageSource.GENERIC, 1.5f);
